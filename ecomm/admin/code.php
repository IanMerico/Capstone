<?php
    ...
    function isFileExists($filename, $directory) {
    ...
    if(isset($_POST['add_category_btn'])) {
        ...
        $category_query = "INSERT INTO categories (name, slug, description,  status, popular, image, created_at) VALUES ('$name', '$slug', '$description', '$status', '$popular', '$filename', NOW())";
        ...
        $producy_query = "INSERT INTO products (category_id, name, slug, small_description, description, original_price, selling_price, fee, qty,  status, trending, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($con, $producy_query);

        mysqli_stmt_bind_param($stmt, "isssssssssss", $category_id, $name, $slug, $small_description, $description, $original_price, $selling_price, $fee, $qty, $status, $trending, $data);
        ...
        $existing_images = explode(" ", $old_image);
    
        // Check if any new image file has the same name as an existing image file
        foreach ($new_image as $image) {
            if (in_array($image, $existing_images)) {
                // Image already exists, handle the error or display a message
                redirect("edit_products.php?id=$product_id", "This Image already exists. Please choose a different image.");
                exit; // Stop execution if needed
            }
        }
    
        $image_names = [];
        if (!empty(array_filter($new_image))) {
            foreach ($_FILES['image']['name'] as $key => $val) {
                $image = $_FILES['image']['name'][$key];
                $file_temp = $_FILES['image']['tmp_name'][$key];
            ...
            // Update the image names in the database
            $images = implode(" ", $image_names);
        } else {
            // No new image selected, keep the existing image names
            $images = $old_image;
        }
    
        // $update_query_products = "UPDATE products SET category_id=?, name=?, slug=?, small_description=?, description=?, original_price=?, selling_price=?, fee=?, qty=?, meta_title=?, meta_description=?, status=?, trending=?, image=? WHERE id=?";

        $update_query_products = "UPDATE products SET category_id=?, name=?, slug=?, small_description=?, description=?, original_price=?, selling_price=?, fee=?, qty=?, status=?, trending=?, image=? WHERE id=?";

        $stmt = mysqli_prepare($con, $update_query_products);
    
        // mysqli_stmt_bind_param($stmt, 'isssssssssssssi', $category_id, $name, $slug, $small_description, $description, $original_price, $selling_price, $fee, $qty, $meta_title, $meta_description, $status, $trending, $images, $product_id);

        mysqli_stmt_bind_param($stmt, 'isssssssssssi', $category_id, $name, $slug, $small_description, $description, $original_price, $selling_price, $fee, $qty, $status, $trending, $images, $product_id);
        ...
        if ($update_query_products_run) {
            if (!empty(array_filter($new_image))) {
                ...
                foreach ($existing_images as $existing_image) {
                    if (file_exists("../uploads/" . $existing_image)) {
    ...
    redirect("view_order.php?t=$track_no", "Order Status Updated Successfully");
        ...
        $old_image = $_POST['old_business_logo'];
            ...
            // Check if a new image is uploaded and if it already exists
            if ($new_image != "") {
                if (isFileExists($new_image, $path)) {
                    redirect("system_info.php", "The uploaded image already exists. Please choose a different image.");
        ...
        $old_cover = implode(',', $old_covers);
          ...
          // Check if any of the cover images already exist
        if (!empty($_FILES['cover']['name'])) {
            $cover_count = count($_FILES['cover']['name']);
            for ($i = 0; $i < $cover_count; $i++) {
                $cover_name = $_FILES['cover']['name'][$i];
                if (isFileExists($cover_name, $paths)) {
                    redirect("system_info.php", "One or more uploaded cover images already exist. Please choose different images.");
                    exit; // Stop execution if validation fails
                }
            }
        }
        ...
        $update_query = "UPDATE system_info SET system_name='$system_name', seller_name='$seller_name', business_name='$business_name', seller_email='$seller_email', business_address='$business_address', business_logo='$update_filename', meta_title='$meta_title', meta_description='$meta_description', meta_keywords='$meta_keywords', cover='$update_cover_filenames_str' WHERE id='$systeminfo_id'";
        ...
        if($update_query_run) {
            ...
            redirect("user.php?id=$user_id", "user updated successfully");
        } else {
            redirect("edit_user.php?id=$category_id", "Something went wrong!");
        }
